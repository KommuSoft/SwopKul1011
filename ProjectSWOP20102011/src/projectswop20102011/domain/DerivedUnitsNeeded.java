...
class DerivedUnitsNeeded extends UnitsNeeded {
    ...
    public UniqueList<Unit> getPolicyProposal(Set<Unit> availableUnits) {
        UniqueList<Unit> units = null;
        if (getSendable().getEmergencies().size() > 0) {//TODO: dit geldt toch altijd (is een invariant voor disaster.)
            units = new UniqueList<Unit>();
            for (Emergency e : getSendable().getEmergencies()) {
                units.addAll(e.calculateUnitsNeeded().getPolicyProposal(availableUnits));//TODO: moet hier geen getUnitsNeeded() <-> calculateUnitsNeeded() komen te staan????
            }
        }
