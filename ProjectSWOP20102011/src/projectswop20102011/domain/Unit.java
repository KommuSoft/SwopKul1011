	...
	private void changeLocation(long duration) throws InvalidDurationException {
		...
		if (duration > 0) {
			if (getDestination() != null) {
				...
				final long startY = getCurrentLocation().getY();
				final long goalX = getDestination().getX();
				final long goalY = getDestination().getY();

				final double distance = getCurrentLocation().getDistanceTo(getDestination());
				final long distanceX = goalX - startX;
				final long distanceY = goalY - startY;

				final long stopX = Math.round(startX + (distanceX / distance) * getSpeed() * duration / 3600);
				final long stopY = Math.round(startY + (distanceY / distance) * getSpeed() * duration / 3600);
				final GPSCoordinate stop = new GPSCoordinate(stopX, stopY);

				final double calculatedDistance = getCurrentLocation().getDistanceTo(stop);
