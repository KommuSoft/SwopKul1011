...
import projectswop20102011.eventhandlers.AssignmentEventHandler;
import projectswop20102011.eventhandlers.RemoveAssignmentEventHandler;
...
public class Main {
	...
	private static MainUserInterface initUserInterface(World world, IExternalSystem es) throws Exception {
		...
		CommandUserInterface dispatchUnitsUserInterface = new DispatchUnitsToEmergencyUserInterface(new DispatchUnitsToEmergencyController(world, new AssignmentEventHandler(es)), new InspectEmergenciesController(world), new EmergencyMapper(world));
		CommandUserInterface selectHospitalUserInterface = new SelectHospitalUserInterface(new SelectHospitalController(world));
		CommandUserInterface endOfEmergencyUserInterface = new EndOfTaskUserInterface(new EndOfTaskController(world, new RemoveAssignmentEventHandler(es)));
		CommandUserInterface timeAheadUserInterface = new TimeAheadUserInterface(new TimeAheadController(world, es));
		CommandUserInterface removeUnitAssignmentFromEmergencyInterface = new RemoveUnitAssignmentFromEmergencyUserInterface(new RemoveUnitAssignmentFromEmergencyController(world, new RemoveAssignmentEventHandler(es)), new EmergencyMapper(world));
		CommandUserInterface removeUnitAssignmentFromDisasterInterface = new RemoveUnitAssignmentFromDisasterUserInterface(new RemoveUnitAssignmentFromDisasterController(world, new RemoveAssignmentEventHandler(es)), new DisasterMapper(world));
		CommandUserInterface dispatchUnitsToDisasterInterface = new DispatchUnitsToDisasterUserInterface(new DispatchUnitsToDisasterController(world, new AssignmentEventHandler(es)), new DisasterMapper(world));
